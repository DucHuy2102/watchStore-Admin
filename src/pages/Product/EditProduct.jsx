...
import { Button, Card, Form, Input, InputNumber, Select, Upload, message, Spin, Image } from 'antd';
import {
    InboxOutlined,
    InfoCircleOutlined,
    ProfileOutlined,
...
export default function CreateProduct() {
        ...
        const fetchProduct = async () => {
            try {
                ...
                form.setFieldsValue({
                    ...
                    wireMaterial: productData.wireMaterial,
                    shellMaterial: productData.shellMaterial,
                    ...
                    genderUser: productData.genderUser,
                ...
                if (productData.color && Array.isArray(productData.color)) {
                    const colorObjects = productData.color.map((hex) => ({
                        hex: hex,
                        name: colorNames[hex] || 'Màu tùy chỉnh',
                    }));
                    setSelectedColors(colorObjects);
                }

                if (productData.color) {
                    const colors = productData.color.map((hex) => ({
                        hex,
                        name: colorNames[hex] || 'Màu tùy chỉnh',
                    }));
    ...
    const onFinish = async (values) => {
        try {
            ...
            console.log('Lưu sản phẩm thành công:', res);
                ...
                >
                    ...
                    >
                        ...
                        <div className='grid grid-cols-3 gap-4 mt-4'>
                            <Upload
                                listType='picture-card'
                                fileList={product.img?.map((url) => ({ thumbUrl: url }))}
                                showUploadList={{
                                ...
                                onRemove={(file) => {
                                    const index = product.img?.indexOf(file.thumbUrl);
                                    if (index !== -1) {
